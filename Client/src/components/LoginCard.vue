  ...
  <div class="login-panel w-full max-w-md space-y-6 text-white">
    <div class="space-y-2 text-center">
      ...
      <h2 class="text-xl font-semibold text-white">
        {{ isLoginMode ? 'Login to your account' : 'Sign up your account' }}
      </h2>
      <p class="text-xs text-white/50">
        {{ isLoginMode ? 'Welcome back. Login with email or social account.' : 'Create your customer account for reservations and ordering.' }}
      </p>
    </div>
    ...
    <div class="grid grid-cols-2 gap-2 border border-gold/15 bg-black/60 p-1">
      <button 
        ...
        class="h-10 text-xs font-bold uppercase tracking-[0.16em] transition-all"
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="space-y-2">
        ...
        <input
          ...
          v-model="email"
          type="email"
          placeholder="customer@nekmak.com"
          class="login-input h-12 w-full border border-gold/15 bg-black/60 px-4 text-sm text-white outline-none transition-all placeholder:text-white/25 focus:border-gold focus:bg-black"
          required
        />
      </div>
    ...
    <div class="flex items-center gap-4">
      ...
      <span class="text-[10px] uppercase tracking-[0.16em] text-white/35">Or select method to log in</span>
      <div class="h-px flex-1 bg-gold/20"></div>
    </div>
